...
int cBus(vector<vector<int>>& M, int n, int k, int visited_all, int mask, vector<vector<int>>& dp, int currIndex, int count) {
    if (mask == visited_all)
...
int main() {
    ...
    cin >> n >> k;
    int i, j;
    vector<vector<int>> M(2 * n + 1, vector<int>(V, 0));
    for (i = 0; i < 2 * n + 1; i++)
        for (j = 0; j < 2 * n + 1; j++)
            cin >> M[i][j];

    int N = 2 * n + 1;
    vector<vector<int>> dp(1 << N, vector<int>(V, -1));
